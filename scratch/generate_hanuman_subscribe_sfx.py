...
# --- 2. BAJRANGBALI GADA HEAVY SWISH & BRASS MANJIRA (1.5s - 2.4s) ---
t2 = t[(t >= 1.5) & (t < 2.4)] - 1.5
# Low pitch Gada heavy air whoosh (sweeping 180Hz -> 60Hz)
pitch_whoosh = 60.0 + (180.0 - 60.0) * np.exp(-10 * t2)
whoosh_phase = 2 * np.pi * np.cumsum(pitch_whoosh) / sample_rate
whoosh = 0.60 * np.sin(whoosh_phase) * np.exp(-5 * t2)

# Brass Manjira Chime
manjira = 0.45 * np.sin(2 * np.pi * 2640 * t2) * np.exp(-4.5 * t2)

audio[(t >= 1.5) & (t < 2.4)] += (whoosh + manjira) * 0.75
...
wav_out = r'C:\Users\user\OneDrive\Desktop\creatorsfree.in\creators-free-website\media\videos\lord_hanuman_gada_subscribe_vfx\1080p60\hanuman_subscribe_sfx.wav'
wavfile.write(wav_out, sample_rate, audio_int16)
print(f'Successfully generated Lord Hanuman Gada Subscribe SFX WAV at: {wav_out}')
