/**
 * Mobile Step-by-Step Resume Builder Engine - creatorsfree.in
 * 4-Step guided wizard with persistent mobile view switcher (Edit Form vs Live Preview).
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupViewToggle() {
    ...
    document.querySelectorAll('.link-view-preview').forEach(link => {
      link.addEventListener('click', (e) => { e.preventDefault(); showPreviewMode(); });
    });

    document.querySelectorAll('.link-back-edit').forEach(link => {
      link.addEventListener('click', (e) => { e.preventDefault(); showEditMode(); });
    });
  }
  ...
  function setupStepNavigation() {
    ...
    stepPills.forEach(pill => {
      pill.addEventListener('click', () => {
        const stepNum = parseInt(pill.dataset.step);
  ...
  function updateStepUI() {
    ...
    // Update Step Pills
    stepPills.forEach(pill => {
      const sNum = parseInt(pill.dataset.step);
      pill.classList.remove('active', 'completed');
      if (sNum === state.currentStep) {
        pill.classList.add('active');
      } else if (sNum < state.currentStep) {
    ...
    // Update Nav Buttons
    if (btnPrev) {
      btnPrev.style.display = state.currentStep === 1 ? 'none' : 'inline-flex';
    }

    if (btnNext) {
      if (state.currentStep === 4) {
        btnNext.textContent = '👁️ Preview & Download Resume';
        btnNext.onclick = () => {
          if (togglePreviewBtn) togglePreviewBtn.click();
        };
      } else {
        btnNext.textContent = `Next Step (Step ${state.currentStep + 1} of 4) →`;
  ...
  function copyShareLink() {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(window.location.href).then(() => {
        alert('🔗 Resume Builder link copied to clipboard!');
      });
    } else {
    ...
    state.education.forEach((edu, idx) => {
      ...
      div.innerHTML = `
        ...
        <div class="field-row" style="margin-bottom:8px;">
          <div><label>Degree / Course</label><input type="text" value="${edu.degree}" class="edu-degree" data-idx="${idx}" placeholder="e.g. B.Tech / 12th"></div>
  ...
  function renderDynamicExpRows() {
    ...
    state.experience.forEach((exp, idx) => {
      ...
      div.innerHTML = `
        <button type="button" class="btn-remove" data-idx="${idx}">Remove</button>
        <div class="field-row" style="margin-bottom:8px;">
          <div><label>Job Title</label><input type="text" value="${exp.title}" class="exp-title" data-idx="${idx}" placeholder="e.g. Software Intern"></div>
          <div><label>Company Name</label><input type="text" value="${exp.company}" class="exp-comp" data-idx="${idx}" placeholder="e.g. TCS / Tech Mahindra"></div>
        </div>
        <div style="margin-bottom:8px;"><label>Duration</label><input type="text" value="${exp.duration}" class="exp-dur" data-idx="${idx}" placeholder="e.g. Jan 2023 - Present"></div>
        <div><label>Key Responsibilities</label><textarea class="exp-det" data-idx="${idx}">${exp.details}</textarea></div>
      `;
  ...
  function renderDynamicProjRows() {
    ...
    state.projects.forEach((proj, idx) => {
      ...
      div.innerHTML = `
        <button type="button" class="btn-remove" data-idx="${idx}">Remove</button>
        <div class="field-row" style="margin-bottom:8px;">
          <div><label>Project Title</label><input type="text" value="${proj.title}" class="proj-title" data-idx="${idx}" placeholder="e.g. E-Commerce Portal"></div>
        ...
        <div><label>Project Description</label><textarea class="proj-det" data-idx="${idx}">${proj.details}</textarea></div>
      `;
